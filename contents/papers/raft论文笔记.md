Raft论文笔记: In search of an Understandable Consensus Algorithm
===============================================================
摘要
---
...
1、介绍
------
一致性算法允许一组servers以一致的group工作，允许部分成员失败。因此它成为构建大规模软件系统的关键角色。Paxos一直主宰着一致性算法，很多算法都是基于Paxos或者他的衍生版本。但是Paxos太难了，Paxos需要经过复杂的修改才能适用真实系统。  
Raft易懂易建,最主要目标是易懂。重要的不是它work，而是为什么可以work。
...
2、复制状态机
-----------
一致性算法是在`复制状态机`的背景下提出来的。一组servers集上的状态机会为相同的状态计算相同的副本，这样即使有些server坏掉也可以继续操作。复制状态机用来解决分布式系统中失败容忍的一系列问题。复制状态机一种典型实现是复制日志，如下图：
![status](../../images/raft/status.png)
每个服务存储一个包含一系列命令的日志，他们的状态机顺序的执行。每个日志在相同顺序上包含相同的命令，因此每个状态机处理相同命令序列。
因为状态机是确定的，则计算的状态和输出都是相同。

一致性算法的任务保证复制日志的一致性，server上的一致性模块接收客户端命令，并且把它们加入到log中。不同server间的一致性模块相互通信，来确认每个日志最终以相同的顺序包含相同的请求，即使有些服务失败。
一旦命令被正确的复制，每个服务的状态机以log顺序处理他们，并且把命令执行的输出返回给client。
